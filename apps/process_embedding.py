...
def _load_and_transform_data(spark: SparkSession, logger) -> Optional[DataFrame]:
    """Loads gold layer data, transforms it for embedding, and returns a Spark DataFrame."""
    # Read data from Gold Delta Tables
    logger.info(f"Reading 'top_complaints' from: {constants.EMBEDDING_PATH_TOP_COMPLAINTS}")
    if not os.path.exists(constants.EMBEDDING_PATH_TOP_COMPLAINTS):
        logger.error(f"Path does not exist - {constants.EMBEDDING_PATH_TOP_COMPLAINTS}")
        return None
    top_complaints_df = spark.read.format("delta").load(constants.EMBEDDING_PATH_TOP_COMPLAINTS)

    logger.info(f"Reading 'by_borough' from: {constants.EMBEDDING_PATH_BY_BOROUGH}")
    if not os.path.exists(constants.EMBEDDING_PATH_BY_BOROUGH):
        logger.error(f"Path does not exist - {constants.EMBEDDING_PATH_BY_BOROUGH}")
        return None
    ...
    logger.info("Transformed data into document format with IDs and metadata.")
...
def _ingest_partition_to_chromadb(iterator: iter) -> None:
    ...
    while True:
        ...
        print(f"Ingesting batch of {len(ids_to_ingest)} documents from a partition.", file=sys.stderr)
...
def main() -> None:
    ...
    logger = get_logger(spark, "Embedding Layer")

    logger.info(EMBEDDING_BANNER)
    logger.info("--- Starting Embedding Layer Processing ---")

    try:
        all_docs_df = _load_and_transform_data(spark, logger)
        if all_docs_df is not None:
            # Ensure the collection is in a clean state before ingesting new data.
            logger.info(f"Preparing collection: '{constants.EMBEDDING_COLLECTION_NAME}'")
            chroma_client = chromadb.HttpClient(host=constants.EMBEDDING_CHROMA_HOST, port=constants.EMBEDDING_CHROMA_PORT)
            ...
            if constants.EMBEDDING_COLLECTION_NAME in existing_collections:
                logger.info(f"Collection '{constants.EMBEDDING_COLLECTION_NAME}' found. Deleting it to ensure a fresh start.")
                ...
                logger.info("Collection does not exist. Proceeding with creation during ingestion.")

            # Use foreachPartition to process data on each worker node
            logger.info("Starting distributed ingestion into ChromaDB...")
            all_docs_df.foreachPartition(_ingest_partition_to_chromadb)
            logger.info("Embedding Layer: Data Ingestion Complete!")

            # Verify final count by querying ChromaDB directly from the driver
            logger.info("Verifying total document count in ChromaDB...")
            collection = chroma_client.get_collection(name=constants.EMBEDDING_COLLECTION_NAME)
            logger.info("--------------------------------------------------")
            logger.info(f"Total documents in dataframes: {all_docs_df.count()}")
            logger.info(f"Total documents in collection '{constants.EMBEDDING_COLLECTION_NAME}': {collection.count()}")
            logger.info("--------------------------------------------------")
        ...
        logger.info("--- Embedding Layer Processing Finished ---")
        spark.stop()
